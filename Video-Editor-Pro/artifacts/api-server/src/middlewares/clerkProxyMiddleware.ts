...
function getClerkFapiUrl(): string {
  ...
  // base64 decodes to <domain>$
  const match = publishableKey.match(/^pk_(?:test|live)_(.+)$/);
  if (!match) return "https://frontend-api.clerk.dev";
  try {
  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        // For localhost (no reverse proxy in front), there is no
        // x-forwarded-proto header — default to http, not https.
        const isLocalhost = host.startsWith("localhost") || host.startsWith("127.");
        const forwardedProto = req.headers["x-forwarded-proto"];
        const proto = Array.isArray(forwardedProto)
          ? forwardedProto[0]
          : forwardedProto || (isLocalhost ? "http" : "https");
        const proxyUrl = `${proto}://${host}${CLERK_PROXY_PATH}`;
